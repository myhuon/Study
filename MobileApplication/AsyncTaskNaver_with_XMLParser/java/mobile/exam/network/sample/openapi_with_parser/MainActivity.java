...
public class MainActivity extends AppCompatActivity {
    ...
    public void onClick(View v) {
        switch(v.getId()) {
            case R.id.btnSearch:
                ...
                new NaverAsyncTask().execute(apiAddress, query); //api 주소, 검색어
                break;
        }
    }
    ...
    class NaverAsyncTask extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... strings) {
            String address = strings[0]; // api 주소
            String query = strings[1]; // 검색어

            String apiURL = null;
            try {
                apiURL = address + URLEncoder.encode(query, "UTF-8"); // api 주소 + 쿼리
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }

            String result = downloadNaverContents(apiURL); // xml 형식의 String 반환
            return result;
        }


        @Override
        protected void onPostExecute(String result) { // result는 xml 형식의 String이다.
            Log.i(TAG, result);
            progressDlg.dismiss();

            ArrayList<NaverBookDto> parserdList = parser.parse(result);     // 오픈API 결과의 파싱 수행 ( dto 객체 반환 )

            if (parserdList.size() == 0) {
                Toast.makeText(MainActivity.this, "No data!", Toast.LENGTH_SHORT).show();
            } else {
                resultList.clear(); // 기존에 있던 데이터 삭제
                resultList.addAll(parserdList); // list View에 parsing된 데이터 리스트 출력
